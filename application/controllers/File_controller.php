...
class File_controller extends Home_Core_Controller
{
    ...
    //download convert demo file convert
    public function download_convert_demo_file()
    {
        if (!$this->auth_check) {
            redirect($this->agent->referrer());
        }
        $id = $this->input->post('id', true);
        $file = $this->convert_model->get_convert_file($id);
        if (!empty($file)) {
            if (($file->buyer_id == $this->auth_user->id) ) {
                $this->load->helper('download');
                force_download(FCPATH . "uploads/convert-files/".$file->buyer_id.'/' . $file->file_aab, NULL);
            }
        }
        redirect($this->agent->referrer());
    }

    //download purchased digital file Convert
    public function download_purchased_digital_file_convert()
    {
        post_method();
        if (!$this->auth_check) {
            redirect($this->agent->referrer());
        }
        $sale_id = $this->input->post('product_id', true);

        $file = $this->convert_model->get_convert_file($sale_id);
        if (!empty($file)) {
            if ($file->buyer_id == $this->auth_user->id) {
//                $file = $this->file_model->get_product_digital_file($sale->product_id);
                $this->load->helper('download');
                @ force_download(FCPATH . "uploads/convert-files/".$file->buyer_id.'/' . $file->file_aab_apk, NULL);
