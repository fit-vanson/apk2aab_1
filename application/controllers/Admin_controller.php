...
class Admin_controller extends Admin_Core_Controller
{
  ...
  */
    ...
    {
        check_permission('convert_files');
        $this->load->model('convert_model');
        $data['title'] = trans("convert_files");
        $data['form_action'] = admin_url() . "convert-files";

        //get paginated requests
        $pagination = $this->paginate(admin_url() . 'convert-files', $this->convert_model->get_admin_convert_files_count());


        $data['convert_files'] = $this->convert_model->get_admin_paginated_convert_files($pagination['per_page'], $pagination['offset']);
        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/convert/convert_files', $data);
        $this->load->view('admin/includes/_footer');
    }
     ...
     * Submit Convert File
     */
    public function submit_convert_post()
    {
        check_permission('convert_files');
        $this->load->model('convert_model');
        $id = $this->input->post('id', true);
        ...
        if ($this->convert_model->submit_quote($convert_request)) {
            //send email
            $buyer = get_user($convert_request->buyer_id);
            if (!empty($buyer) && $this->general_settings->send_email_bidding_system == 1) {
                $email_data = array(
                    'email_type' => 'email_general',
                    'to' => $buyer->email,
                    'subject' => trans("quote_request"),
                    'email_content' => trans("your_quote_request_replied") . "<br>" . trans("quote") . ": " . "<strong>#" . $convert_request->id . "</strong>",
    ...
    /**
     * Delete Convert Request
     */
    public function delete_convert_files_post()
    {
        check_permission('convert_files');
        $this->load->model('convert_model');
        $id = $this->input->post('id', true);
        if ($this->convert_model->delete_admin_convert_file($id)) {
            $this->session->set_flashdata('success', trans("msg_deleted"));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
    ...
    */
    ...
    /**
     * Telegram Notification Post
     */
    public function telegram_notification_post()
    {
        check_permission('general_settings');
        if ($this->settings_model->update_telegram_notification()) {
            $this->session->set_flashdata('success', trans("msg_updated"));
            $this->session->set_flashdata("mes_telegram_notification", 1);
            redirect($this->agent->referrer());
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
            $this->session->set_flashdata("mes_telegram_notification", 1);
