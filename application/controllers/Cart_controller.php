...
class Cart_controller extends Home_Core_Controller
{
    ...
    {
        $file_request_id = $this->input->post('id', true);
        if (!empty($this->cart_model->add_to_cart_file($file_request_id))) {
            redirect(generate_url("cart"));
        }
        redirect($this->agent->referrer());
    }
...
//        if($this->session->mds_shopping_convert_cart){
//            $data['cart_items'] = $this->session->mds_shopping_convert_cart;
//            $data['mds_payment_type'] = "sale";
//            $data['cart_total'] = (object)([
//                'subtotal' => $this->session->mds_shopping_convert_cart->total_price,
//                'vat' => 0,
//                'shipping_cost' => 0,
//                'total_before_shipping' => $this->session->mds_shopping_convert_cart->total_price,
//                'total' => $this->session->mds_shopping_convert_cart->total_price,
...
//            $data['cart_has_digital_product'] = $this->cart_model->check_cart_has_digital_product();
//            $data['show_shipping_cost'] = 1;
//        $data['total_amount'] = $this->session->mds_shopping_convert_cart->total_price;
