    ...
    public function add_order_offline_payment($payment_method)
    {
        ...
        if (!empty($cart_total)) {
            ...
            if ($this->db->insert('orders', $data)) {
                ...
                $this->convert_model->set_convert_quotes_as_completed_after_purchase();
            ...
            foreach ($cart_items as $cart_item) {
                ...
                if($cart_item->product_id == 'convert_file'){
                    ...
                    $data = array(
                        ...
                        'buyer_id' => 0,
                        'buyer_type' => "guest",
                        'product_id' => $cart_item->quote_request_id,
                        'product_type' => 'digital_convert_file',
                        'listing_type' => 'bidding',
                        'product_title' => $cart_item->product_title,
                        'product_slug' => 'convert_file',
...
//                        $this->db->where('id', $product->id)->update('products', $array);
    ...
    //add digital file convert
    public function add_digital_file_convert($product_id, $order_id)
    {

        $product_id = clean_number($product_id);
        $order_id = clean_number($order_id);
        $product = get_active_file_convert($product_id);
        $order = $this->get_order($order_id);
