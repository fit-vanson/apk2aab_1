...
class Upload_model extends CI_Model
{
    ...
    //convert file upload
    public function convert_file_upload($input_name, $file_name,$user_id)
    {
        $directory_path = FCPATH . 'uploads/convert-files/'.$user_id.'/';
        if (!is_dir($directory_path)) {
            //Create directory.
            @mkdir($directory_path, 0755, true);
        }

        $config['upload_path'] = './uploads/convert-files/'.$user_id.'/';
        $config['allowed_types'] = '*';
        $config['file_name'] = $file_name;
