...
class File_model extends CI_Model
{
    ...
    */
    ...
    public function upload_convert_files($user_id)
    {
        if (isset($_FILES['file'])) {
            if (empty($_FILES['file']['name'])) {
                exit();
...
//            $file_name = str_slug($this->general_settings->application_name) . "-convert-file-" . $user->id . uniqid() . "." . $ext;
            $file_name = str_slug($name) . $ext;


            if ($this->upload_model->convert_file_upload('file', $file_name,$user_id)) {
                $data = array(
//                    'seller_id' => user()->id,
                    'buyer_id' => $user_id,
                    'file_apk' => $file_name,
                    'status' => 'new_convert_request',
                    'created_at' => date('Y-m-d H:i:s'),
                    'updated_at' => date('Y-m-d H:i:s')
                );
                @$this->db->close();
                @$this->db->initialize();
                $this->db->insert('convert_requests', $data);
