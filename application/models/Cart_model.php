...
class Cart_model extends CI_Model
{
    ...
    //add to cart file
    public function add_to_cart_file($file_request_id)
    {
        $this->load->model('convert_model');
        $quote_request = $this->convert_model->get_convert_file($file_request_id);
...
//            if (!empty($product)) {
                ...
                $item = new stdClass();
                $item->cart_item_id = generate_unique_id();
                $item->product_id = "convert_file";
                $item->product_type = 'digital';
                $item->product_title = 'Convert file apk2aab';
                $item->options_array = array();
                $item->quantity = 1;
                $item->unit_price = $quote_request->price_offered;
        ...
        if (!empty($cart)) {
            foreach ($cart as $cart_item) {
                if($cart_item->product_id == 'convert_file'){
                    $this->load->model('convert_model');
//                    $quote_request = $this->convert_model->get_quote_request($cart_item->quote_request_id);
                    $quote_request = $this->convert_model->get_convert_file($cart_item->quote_request_id);
                    ...
                    if (!empty($quote_request) && $quote_request->status == 'pending_payment') {
                        ...
                        $item->product_image = base_url() . 'assets/img/no-image.jpg';
                        $item->options_array = $cart_item->options_array;
                        $item->quantity = $cart_item->quantity;
                        $item->unit_price = $price_offered ;
                        ...
                        $item->seller_id = $quote_request->buyer_id;
                        $item->shipping_class_id = 0;
                        $item->is_stock_available = 1;
                        array_push($new_cart, $item);
                    }
